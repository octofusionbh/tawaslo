...
## SCHEDULING BUG — ROOT CAUSE FOUND (2026-07-22): function timeout, not the cron pinger
Manual trigger of /api/cron?key=... returned 504 FUNCTION_INVOCATION_TIMEOUT. Root cause: api/cron.js had `maxDuration: 10` (10s), but publishing a REAL post calls /api/meta-publish which polls Instagram's media-container status in 3s intervals (waitReady) → real image/video publish takes >10s → Vercel kills the function mid-publish → post stuck (claimed 'publishing' or left 'scheduled'), never marked 'published'. Earlier "successes" were fast FAILURES (text-only + expired token errored <10s). Diagnosis: appr_token NULL (not an approval-gate issue); posts were also future-dated (Jul 26–31) which was a red herring.
FIX (needs redeploy): api/cron.js maxDuration 10 → 60; vercel.json adds api/meta-publish.js maxDuration 60 (it had no config = default 10s). node --check OK.
AFTER DEPLOY: (1) reset stuck posts `update posts set status='scheduled' where status='publishing';`; (2) re-hit cron URL — should return JSON not 504; (3) ensure cron-job.org job is still ENABLED (may auto-disable after repeated 504s). Consider later: auto-reset stale 'publishing' posts in cron; make publish async for very slow video.

## GO-LIVE PREP — Instagram connect scope fix (2026-07-22)
Business/Tech-Provider verification = DONE (Octo Fusion verified). Core IG perms approved (basic/content_publish/manage_insights = "Ready to publish"). Comments + messages (Inbox) = App Review REJECTED.
FIX (applied, build green): src/TawasaloApp.js ~8269 connectInstagram scope now requests ONLY approved perms; comments+messages gated behind `REACT_APP_IG_INBOX=1` (mirrors the existing REACT_APP_META_IG pattern for the FB button). Prevents "invalid scopes" auth failure for real customers once Live. Backup: outputs/deai/TawasaloApp.bak5.js
TO GO SELLABLE (no Meta wait left): (1) push this; (2) in Meta app: publish the 3 approved perms to Advanced Access; (3) flip app Dev→Live. Then agencies connect + publish + insights immediately.
INBOX LATER: get the rejection reason for instagram_business_manage_comments + _manage_messages → rewrite submission + screencast → resubmit (App Review runs fine on a Live app, parallel, zero downtime) → on approval set REACT_APP_IG_INBOX=1 + redeploy.

## 🚀 APP PUBLISHED / LIVE (2026-07-22)
Meta app flipped Dev → LIVE ("Published"). App is public. Approved perms (instagram_business_basic/content_publish/manage_insights + pages_show_list/pages_read_engagement/public_profile) are at Advanced Access → real customers (agencies) can now connect Instagram + publish + see insights. Verification/Tech-Provider done. Scope fix deployed (connect requests approved perms only; Inbox scopes behind REACT_APP_IG_INBOX flag).
STILL TESTING-ONLY until App Review (batch later): Inbox (manage_comments/manage_messages — rejected, resubmit), Meta Ads + boosting (ads_management/ads_read — never submitted; finish Ads feature #60 first), business_management (full FB connect).
NEXT VERIFY: connect an Instagram from an account with NO role on the Meta app (true customer test) → confirm authorize + publish works end-to-end in production.

## SCHEDULER — resolved + carousel-on-schedule fix (2026-07-22/24)
- Scheduler root cause was the cron-job.org pinger being disabled by earlier 504s. Re-created the job (www URL + key, every minute, enabled). Cron healthy ("nothing due"), posts publishing across all accounts. CONFIRMED WORKING by user (planner + scheduler). Vercel plan = Hobby (native cron can't do every-minute → external pinger required; consider a 2nd redundant pinger e.g. UptimeRobot 5-min).
- CAROUSEL-ON-SCHEDULE BUG FIXED: scheduled carousels only saved image_url=imgs[0] and dropped per-slide captions → cron published a single image. Fix: (1) SQL tawaslo-carousel-schedule.sql adds posts.image_urls / slide_captions / alt_texts (jsonb); (2) composer scheduled srow now saves image_urls/slide_captions/alt_texts; (3) api/cron.js meta-publish body now passes imageUrls/slideCaptions/altTexts. Build green. RUN SQL FIRST, THEN PUSH.
- Approval flow: per-client "requires approval" toggle lives on ApprovalsPage (require_approval); unapproved posts are held (by design). User dropped the "auto-publish unapproved at scheduled time" idea.

## CONNECT BUG — new customer couldn't connect any account (2026-07-25)
Symptom: subscriber logged in to Instagram + clicked Allow, but nothing saved and NO error shown; social_accounts had no new row. Root cause: the 6 OAuth-callback effects were gated `else if (Xcode && realClientId)` — if realClientId hadn't resolved (fresh account), the callback NEVER ran → silent no-save. resolveClientId already falls back to the owner's first client, but the guard blocked it. FIX: removed `&& realClientId` from all 6 guards (ig/li/tt/tw/yt/gb) so the callback always runs and resolveClientId resolves the client. Build green. Backup: outputs/deai/TawasaloApp.bak_oauth.js. PUSH ONLY (no SQL). After deploy: customer refreshes → reconnects → saves.

## HIDE NOT-LIVE FEATURES FROM SUBSCRIBERS (2026-07-25)
Added SUB_HIDDEN_KEYS = {inbox, ads, whatsapp, business} + featHidden(key,email) helper (top-level, near ADMIN_EMAIL). Gated 2 ways: (1) sidebar AGENCY_NAV2 filters these out unless userEmail===ADMIN_EMAIL (octofusionbh@gmail.com); (2) renderPage guard bounces hidden pages to <AgencyDashboard/> for non-Octo (added userEmail to that component's useApp). Build green. PUSH ONLY, no SQL. To add/remove a hidden feature = edit the one-line Set.
WHY hidden: Inbox (comments/DMs Meta-rejected), Ads (ads_management/read not approved), WhatsApp (not set up), Business Profile (GOOGLE_CLIENT_ID not set). NOT hidden: TikTok = APPROVED/live (task #26) as long as TIKTOK_CLIENT_KEY env is set in Vercel — verify it's set. LinkedIn/X/YouTube/Google self-gate to "Coming soon" on the Connections card already.
