  ...
  if (req.method === 'POST' && req.body && req.body.action) {
    ...
    try {
      if (action === 'load') {
        ...
        const calBase = (rows && rows[0] && rows[0].scheduled_at) ? new Date(rows[0].scheduled_at) : new Date();
        const calY = calBase.getFullYear(), calM = calBase.getMonth();
        return res.status(200).json({ posts, readonly, mode: shareMode, agency, client, brand, month: calBase.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }), expires: 7, firstDow: new Date(calY, calM, 1).getDay(), days: new Date(calY, calM + 1, 0).getDate() });
      }
      if (action === 'respond') {
      ...
      try {
        ...
        const pubRes = await fetch(`${SITE}/api/meta-publish`, {
          ...
          body: JSON.stringify({
            ...
            imageUrl: (post.image_urls && post.image_urls.length > 1) ? null : (media && !isVideo ? media : null),
            imageUrls: (post.image_urls && post.image_urls.length > 1) ? post.image_urls : null,
            slideCaptions: post.slide_captions || null,
            altTexts: post.alt_texts || null,
