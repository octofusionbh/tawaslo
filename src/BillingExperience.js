...
export default function BillingExperience({ dark, live = null, onCheckout = null, onManageAccount = null }) {
  ...
  return <div className="tw-billing-experience" ref={root} data-theme={dark ? 'dark' : 'light'} data-area={selected ? 'review' : area}>
    {selected ? <>
      ...
      <div className="bill-included"><span>In every plan</span><span><Check size={15} aria-hidden="true"/>English & Arabic AI captions</span><span><Check size={15} aria-hidden="true"/>Analytics dashboard</span><span>USD pricing{live?'':' · Preview only'}</span></div>
