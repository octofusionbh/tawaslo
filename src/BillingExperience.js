...
export default function BillingExperience({ dark }) {
  ...
  return <div className="tw-billing-experience" ref={root} data-theme={dark ? 'dark' : 'light'} data-area={selected ? 'review' : area}>
    {selected ? <>
      ...
      <div className="bill-review-layout"><section className="bill-review-options" aria-label="Plan options"><h2>01 <span>Choose your rhythm</span></h2>{cycle}<p className="bill-muted">{period === 'annual' ? 'One payment for 12 months. The monthly equivalent is shown for comparison.' : 'One payment per month. The review total covers one month.'}</p>
        ...
        <p className="bill-note"><Lock size={15} aria-hidden="true"/>Preview only. No payment details are requested.</p>
        ...
        <button className="bill-primary" type="button" onClick={() => setReviewed(true)}><Check size={17} aria-hidden="true"/>Test plan selection</button>
        <div className="bill-review-status" role="status">{reviewed ? <><ShieldCheck size={19} aria-hidden="true"/><span>Selection reviewed.<br/>No charge. No subscription change.</span></> : 'Design preview. This does not open checkout.'}</div>
      </aside></div>
    </> : <>
      <header className="bill-hero"><div className="bill-heading"><span className="bill-kicker"><img src="/logo-transparent.png" alt=""/>Plans & billing</span><h1>A little more room.<br/><em>A lot more possibility.</em></h1><p>Find the right fit for your agency's next chapter.</p></div><div className="bill-account-summary"><span className="bill-kicker">Your account</span><strong>Preview workspace</strong><p>Your live subscription is unchanged.</p><button type="button" onClick={() => {setAccountOpen(true);requestAnimationFrame(()=>root.current?.querySelector('#bill-account-title')?.scrollIntoView({block:'start',behavior:'instant'}));}}>Billing details <ArrowRight size={15} aria-hidden="true"/></button></div></header>
      <nav className="bill-area-tabs" aria-label="Billing areas"><button type="button" aria-pressed={area==='plans'} onClick={()=>setArea('plans')}>Plans</button><button type="button" aria-pressed={area==='credits'} onClick={()=>setArea('credits')}>AI credits <span>5 left</span></button></nav>
      {area === 'plans' ? <>
