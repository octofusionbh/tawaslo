...
import { CAMPAIGN_SAMPLE_MONTH, campaignCounts, campaignsInMonth, campaignStorageKey, postsForCampaign, readCampaignBriefs, saveCampaignBrief, validCampaignMonth } from './campaignsPreviewModel';
...
export default function CampaignsExperience({ dark=false, setDark=()=>{}, mobileWeb=false }) {
  const [cursor,setCursor] = useState(locationMonth);
  const [loaded,setLoaded] = useState(()=>readCampaignBriefs(cursor));
  const [planner,setPlanner] = useState(()=>readPlannerMonth(cursor));
  ...
  const visibleCampaigns=campaignsInMonth(loaded.campaigns,cursor);
  ...
  const canCreate=!mobileWeb && !loaded.error;
  ...
  useEffect(()=>{
    const restore=()=>{
      ...
      const next=locationMonth();setCursor(next);setLoaded(readCampaignBriefs(next));setPlanner(readPlannerMonth(next));
  ...
  function remember(date,id=null) {
    const url=new URL(window.location.href);url.searchParams.set('page','campaigns');url.searchParams.set('occasions','editorial');url.searchParams.set('campaignMonth',monthKey(date));
  ...
  function changeMonth(next) {
    ...
    setCursor(next);setLoaded(readCampaignBriefs(next));setPlanner(readPlannerMonth(next));setSelected(null);setNotice('');remember(next);
  }
  ...
  return <main className="tw-campaigns-experience" data-campaigns-theme={dark?'dark':'light'}>
    <div className="cp-preview-line"><span>Design preview · Sample briefs · No paid ads</span><button className="cp-icon" type="button" onClick={()=>setDark(!dark)} aria-label={dark?'Use light campaigns theme':'Use dark campaigns theme'}>{dark?<Sun size={18}/>:<Moon size={18}/>}</button></div>
    <header className="cp-heading"><div className="cp-heading-copy"><span className="cp-kicker"><img src="/logo-transparent.png" width="22" height="22" alt=""/>Campaigns / Marina Social Club</span><h1>One idea.<br/><em>Every expression.</em></h1><p>Bring the brief, visuals, posts, and approvals into one clear working campaign.</p></div><div className="cp-heading-art" aria-hidden="true">{heroVisuals.map((post,index)=><div className={`cp-hero-card cp-hero-card-${index+1}`} key={post.id}><Artwork post={post}/></div>)}<div className="cp-hero-note"><span>{campaign?'Active campaign':'Visual campaign worlds'}</span><strong>{campaign?.name||'Plan it. See it. Ship it.'}</strong></div></div><div className="cp-heading-actions"><div className="cp-heading-metrics"><span><strong>{counts.campaigns}</strong> campaign worlds</span><span><strong>{counts.posts}</strong> planned posts</span></div>{canCreate && !creating && <button ref={newRef} type="button" className="cp-button cp-primary" data-publishing-action="true" onClick={openForm}><Plus size={18} aria-hidden="true"/>Build campaign</button>}</div></header>
    ...
    {creating ? <section className="cp-create" aria-labelledby="cp-create-title"><div className="cp-create-intro"><span className="cp-eyebrow">Start with the brief</span><h2 id="cp-create-title">What are we bringing together?</h2><p>Name the idea, set its goal, and give it a date range.</p><p className="cp-safety">This saves a private draft brief in this browser only. Linking posts and the full campaign workflow will be designed later.</p></div><form ref={formRef} onSubmit={save} noValidate>
      ...
      {!campaign&&visibleCampaigns.length>0&&<nav className="cp-index" ref={indexRef} tabIndex={-1} aria-label="Choose a campaign">{visibleCampaigns.map((item,index)=>{const related=postsForCampaign(item,planner.data.posts);return <button type="button" key={item.id} aria-label={`View campaign: ${item.name}`} onClick={()=>openCampaign(item)}><span className="cp-index-top">{item.sample?'Sample brief':'Draft brief'}</span><i className="cp-index-number" aria-hidden="true">0{index+1}</i><div className="cp-index-art" aria-hidden="true">{related[0]?<Artwork post={related[0]}/>:<div className="cp-brief-cover"><Target size={28}/><span>Your campaign brief</span></div>}</div><span className="cp-index-name">{item.name}</span><span className="cp-index-meta">{dateLabel(item.start)} to {dateLabel(item.end)}</span><span className="cp-index-meta">{postCount(related.length)}</span><span className="cp-index-action">Open campaign <ArrowRight size={16} aria-hidden="true"/></span></button>;})}</nav>}
      {!campaign&&!visibleCampaigns.length&&<section className="cp-empty-month" aria-labelledby="cp-empty-title"><div className="cp-empty-copy"><span className="cp-eyebrow">A quiet month</span><CalendarDays size={28} aria-hidden="true"/><h2 id="cp-empty-title">No campaigns in {monthLabel(cursor)}.</h2><p>Campaigns appear here when their date range covers this month. Browse another month without changing or deleting anything.</p><p>The sample campaigns live in August 2026, where you can see how a brief becomes one visual world.</p><button type="button" className="cp-button" onClick={()=>changeMonth(parseCalendarMonth(CAMPAIGN_SAMPLE_MONTH))}>View August 2026 sample campaigns<ArrowRight size={17} aria-hidden="true"/></button></div>{!!emptyVisuals.length&&<div className="cp-empty-visuals" aria-label="Sample campaign visual direction">{emptyVisuals.map((post,index)=><figure key={post.id}><Artwork post={post}/><figcaption><span>0{index+1}</span>{post.title}</figcaption></figure>)}</div>}</section>}
      {campaign&&<section id="cp-detail" ref={detailSectionRef} className="cp-detail" aria-labelledby="cp-detail-title">
        ...
        <div className="cp-feature"><div className="cp-brief"><span className="cp-eyebrow">{campaign.sample?'The campaign brief':'Private draft brief'}</span><h2 id="cp-detail-title" ref={detailRef} tabIndex={-1}>{campaign.name}</h2><p className="cp-goal">{campaign.goal}</p><dl className="cp-facts"><div><dt><CalendarDays size={16} aria-hidden="true"/>Dates</dt><dd>{dateLabel(campaign.start)}<span className="cp-date-to"> to </span>{dateLabel(campaign.end)}</dd></div><div><dt><Target size={16} aria-hidden="true"/>Content progress</dt><dd>{posts.length?`${approved} of ${posts.length} posts approved`:'No posts linked yet'}</dd></div></dl>{posts.length>0&&<><div className="cp-progress" aria-hidden="true"><span style={{width:`${approved/posts.length*100}%`}}/></div><p className="cp-progress-note">Approval is a content status, not a publishing confirmation.</p></>}</div>
      {sheet.length>0?<div className="cp-contact-sheet" aria-label="Campaign artwork">{sheet.map(post=><figure key={post.id}><Artwork post={post}/><figcaption><span>{post.format}</span><Network platform={post.platform}/></figcaption></figure>)}</div>:<div className="cp-empty-brief"><span className="cp-eyebrow">The idea is saved</span><h3>The content comes next.</h3><p>This is a draft brief. Post linking is not available in this design yet. Your existing Planner content stays unchanged.</p><a className="cp-text-link" href={plannerUrl}>Browse Planner<ArrowRight size={17} aria-hidden="true"/></a></div>}</div>
      {posts.length>0&&<section className="cp-post-section" aria-labelledby="cp-post-title"><header><h3 id="cp-post-title">Posts in this campaign <span>{posts.length}</span></h3><a className="cp-text-link" href={plannerUrl}>Open Planner<ArrowRight size={17} aria-hidden="true"/></a></header><ul className="cp-posts">{posts.map(post=>{const stage=plannerStage(post,planner.data);return <li key={post.id}><div className="cp-thumbnail"><Artwork post={post} compact/></div><div className="cp-post-title"><strong>{post.title}</strong><span><Network platform={post.platform}/><span aria-hidden="true">·</span>{post.format}</span></div><span className={`cp-status cp-status-${stage}`}><i aria-hidden="true"/>{PLANNER_STAGES[stage]}</span><time dateTime={`${monthKey(cursor)}-${String(post.day).padStart(2,'0')}T${post.time}:00+03:00`}>{new Date(cursor.getFullYear(),cursor.getMonth(),post.day).toLocaleDateString('en-GB',{day:'numeric',month:'short'})}<span>{post.time} GMT+3</span></time></li>;})}</ul></section>}
