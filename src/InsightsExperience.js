...
export function AnalyticsExperience() {
  ...
  const [toast,notify]=useToast();
  const profile=analyticsProfiles[channel];
  ...
  return <main className="suite-page suite-page--analytics">
    <div className="suite-preview"><span>Analytics · Sample connected data · Updated 8 minutes ago</span><span><Activity size={14}/>Live view</span></div>
    <PageIntro eyebrow="Marina Social Club / Analytics" title={<>Every signal.<br/><em>One clear picture.</em></>} copy="Audience, reach, engagement and content performance—connected across every channel and ready to act on." actions={<button type="button" className="suite-primary" onClick={()=>notify('Analytics export prepared.')}><Download size={16}/>Export report</button>}>
    ...
    <section className="suite-stat-grid suite-stat-grid--analytics" aria-label={`${channel} analytics summary`}><Stat label="Total audience" value={profile.audience} trend="+4.8%" note=" net growth"/><Stat label="Accounts reached" value={profile.reach} trend="+12.4%" note={` · ${channel}`} tone="aqua"/><Stat label="Impressions" value={profile.impressions} trend="+14.1%" note=" content displayed" tone="coral"/><Stat label="Engagement rate" value={profile.engagement} trend="+0.9 pts" note=" versus prior period" tone="gold"/><Stat label="Link clicks" value={profile.clicks} trend="+18%" note=" tracked destinations" tone="aqua"/><Stat label="Video views" value={profile.views} trend="+22%" note=" three seconds or more" tone="coral"/></section>
...
export function LinksExperience() {
  ...
  const [links,setLinks]=useState([['Weekend collection','tawas.lo/weekend','2,841','Instagram bio'],['Book a consultation','tawas.lo/hello','1,306','LinkedIn'],['New arrivals','tawas.lo/new','984','Campaign'],['Customer feedback','tawas.lo/review','426','Receipt QR']]);
  const [draft,setDraft]=useState('');
  const [toast,notify]=useToast();
  const add=event=>{event.preventDefault();if(!draft.trim())return;setLinks(current=>[[draft.trim(),`tawas.lo/${draft.trim().toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||'link'}`,'0','New'],...current]);setDraft('');setCreating(false);notify('Short link created and ready to copy.');};
  const visible=useMemo(()=>links.filter(row=>row.join(' ').toLowerCase().includes(query.toLowerCase())),[links,query]);
  return <main className="suite-page">
    <div className="suite-preview"><span>Tracked links · Sample click data</span><span><Link2 size={14}/>All links use HTTPS</span></div>
    <PageIntro eyebrow="Marina Social Club / Links" title={<>Short links.<br/><em>Clear journeys.</em></>} copy="Create branded links for posts, bios, QR codes and campaigns—then see what people chose to open." actions={<button type="button" className="suite-primary" onClick={()=>setCreating(true)}><Plus size={16}/>Create link</button>}/>
    <section className="suite-stat-grid"><Stat label="Total clicks" value="5,557" trend="+14%" note=" this month"/><Stat label="Unique visitors" value="4,108" trend="+11%" note=" across all links" tone="aqua"/><Stat label="Top destination" value="Weekend" note=" collection page" tone="coral"/><Stat label="Conversion rate" value="12.6%" trend="+1.8 pts" note=" on tracked goals" tone="gold"/></section>
