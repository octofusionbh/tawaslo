...
function AgencyFloorSetup({ data, setData, onNotice, onHostTest }) {
  ...
  return <section className="rv-agency-floor">
    ...
    <div className="rv-agency-floor-grid">
      <div className="rv-layout-wrap"><div className="rv-water"><span>{activeSpace?.type === 'outdoor' ? 'Open-air edge' : activeSpace?.type === 'floor' ? 'Upper level' : 'Dining room'}</span></div><div className="rv-floor-plan rv-floor-canvas" data-editing="true">{!tables.length && <p className="rv-floor-empty">Add a table to begin this space.</p>}{tables.map(table => <button type="button" key={table.id} className="rv-table rv-table-map" data-shape={table.shape} data-state={table.outOfService ? 'out' : 'free'} aria-pressed={selectedTableId === table.id} aria-label={`${table.name}, ${table.seats} chairs`} style={{ left: `${table.x}%`, top: `${table.y}%`, width: `${table.width}px`, height: `${table.depth}px` }} onClick={() => setSelectedTableId(table.id)}><TableChairs count={table.seats} /><span>{table.name}</span><small>{table.seats} chairs</small><Edit3 size={13} /></button>)}</div></div>
      <aside>{selectedTable ? <><span>Edit table</span><h3>{selectedTable.name}</h3><label>Table name<input value={selectedTable.name} onChange={event => updateTable({ name: event.target.value })} /></label><label>Number of chairs<select value={selectedTable.seats} onChange={event => updateTable({ seats: Number(event.target.value) })}>{Array.from({ length: 20 }, (_, index) => <option value={index + 1} key={index + 1}>{index + 1}</option>)}</select></label><label>Table shape<select value={selectedTable.shape} onChange={event => { const shape = event.target.value; updateTable({ shape, width: shape === 'long' ? 132 : 82, depth: shape === 'long' ? 72 : 82 }); }}><option value="square">Square</option><option value="round">Round</option><option value="long">Long</option></select></label><button type="button" className="rv-oos" aria-pressed={selectedTable.outOfService} onClick={() => updateTable({ outOfService: !selectedTable.outOfService })}><span><strong>{selectedTable.outOfService ? 'Unavailable' : 'Available'}</strong><small>Repairs, private use, or temporary closure</small></span><i><b /></i></button></> : <div className="rv-layout-help"><Table2 size={24} /><strong>Choose a table</strong><p>Edit its name, chairs, shape, or availability.</p></div>}<div className="rv-floor-handoff"><ShieldCheck size={18} /><span><strong>Host handoff</strong><small>This same plan will power daily seating later.</small></span></div></aside>
    </div>
    {spaceOpen && <SpaceModal onClose={() => setSpaceOpen(false)} onAdd={addSpace} />}
  </section>;
}

export default function ReservationsAgencyExperience({ dark = false, setDark = () => {}, onOpenHostTest = () => {} }) {
  ...
  const setupFacts = [
    { label: 'Booking status', value: data.settings.enabled ? 'Open' : 'Paused' },
    { label: 'Venue spaces', value: data.spaces.length },
    { label: 'Mapped tables', value: data.tables.length },
    { label: 'Largest online party', value: data.settings.maxParty },
  ];
  ...
  return <main className="tw-reservations rv-agency-setup" data-reservation-theme={dark ? 'dark' : 'light'}>
    ...
    <header className="rv-heading"><div><span><img src="/logo-transparent.png" width="22" height="22" alt="" />Marina Social Club / Booking setup</span><h1>Bookings, ready before guests arrive.</h1><p>Set availability, venue layout, confirmations, and the guest booking journey. The client’s host team runs daily service later.</p></div><nav><button type="button" onClick={() => setShareOpen(value => !value)}><Link2 size={17} />Link and QR</button><button type="button" onClick={() => setTab('guest')}><Eye size={17} />Preview booking</button><button type="button" className="rv-save" onClick={save}><Save size={17} />Save setup</button></nav></header>
    {shareOpen && <ReservationShare onClose={() => setShareOpen(false)} />}{storageError && <p className="rv-error" role="alert">{storageError}</p>}<div className="rv-toast" role="status">{notice}</div>
    ...
    <nav className="rv-tabs" aria-label="Reservation setup sections">{SETUP_TABS.map(([key, Icon, label]) => <button type="button" key={key} aria-pressed={tab === key} onClick={() => setTab(key)}><Icon size={17} />{label}</button>)}</nav>
    {tab === 'settings' && <AgencyReservationSettings data={data} setData={setData} onSave={save} />}
    {tab === 'floor' && <AgencyFloorSetup data={data} setData={setData} onNotice={setNotice} onHostTest={onOpenHostTest} />}
    {tab === 'guest' && <section className="rv-guest-preview-wrap"><header><span>Guest experience preview</span><strong>Safe preview—no guest is contacted.</strong></header><AgencyGuestBooking data={data} onNotice={message => setNotice(`Preview: ${message}`)} /></section>}
