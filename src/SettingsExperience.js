...
export default function SettingsExperience({ dark, setDark, onNameChange = () => {}, onAgencyLogoChange = () => {}, initial = null, onPersist = null }) {
  ...
  const footer = (key, label) => <div className="set-form-footer">
    <span role="status">{messages[key] || (dirty(key) || (key === 'invoices' && !colorValid) ? 'Unsaved changes. Save before leaving Settings.' : (live?'Profile and agency details save to your account. Appearance and notifications stay in this browser.':'Changes apply to this preview only.'))}</span>
  ...
  const content = {
    profile: <><div className="set-identity"><span className="set-avatar" aria-hidden="true">{draft.profile.name.trim().slice(0, 2).toUpperCase() || 'AB'}</span><div><strong>{draft.profile.name || 'Your profile'}</strong><span>{live?'Workspace member':'Sample workspace member'}</span></div></div>
    ...
    notifications: form('notifications', <><div className="set-rows">{NOTIFICATIONS.map(([key, title, description]) => <label className="set-toggle-row" key={key}><span><strong>{title}</strong><small>{description}</small></span><input type="checkbox" role="switch" checked={draft.notifications[key]} onChange={e => change('notifications', key, e.target.checked)} aria-label={title}/></label>)}</div><p className="set-note">{live?'Saved in this browser. No email or push subscriptions are changed yet.':'Preference preview only. No email or push subscriptions are changed.'}</p></>, 'Save notifications'),
    ...
    about: <div className="set-about"><img src="/logo-transparent.png" alt="Tawaslo"/><h3>A little more space<br/>for your best work.</h3><p>Tawaslo brings your agency's content, clients and conversations together.</p><dl><div><dt>Environment</dt><dd>{live?'Your workspace':'Isolated design preview'}</dd></div><div><dt>Storage</dt><dd>{live?'Profile and agency on the server, the rest in this browser':'This browser, sample workspace only'}</dd></div><div><dt>Live connections</dt><dd>Unchanged</dd></div></dl></div>,
  };
  const descriptions = {
