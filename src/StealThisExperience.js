...
export default function StealThisExperience({dark=false,setDark=()=>{},onOpenStudio=()=>{},onOpenCompetitors=()=>{}}) {
  ...
  const post=inspirationById(route.postId);
  const dirty=Object.keys(drafts).some(id=>drafts[id]!==savedDrafts[id]);
  const base=route.view==='saved'?saved.ideas.map(idea=>inspirationById(idea.id)).filter(Boolean):inspirationPosts(route.platform,route.month);
  const posts=filterInspiration(base,route.format,route.query);
  const isSaved=id=>saved.ideas.some(idea=>idea.id===id);
  const draft=post?(drafts[post.id]??inspirationBrief(post)):'';
  ...
  function saveIdea(item) {const result=saveCompetitorIdea(item);if(!result.ok){setError(result.error);return;}setSaved({accounts:result.accounts,ideas:result.ideas,error:''});setError('');setNotice(`“${item.title}” saved. Find it in Saved ideas or Competitor Insights.`);}
  ...
  return <main className="tw-steal-experience" data-inspiration-theme={dark?'dark':'light'}>
    <div className="sp-preview"><span>Design preview · Fictional examples · No live scans</span><button className="sp-icon" aria-label={dark?'Use light inspiration theme':'Use dark inspiration theme'} onClick={()=>setDark(!dark)}>{dark?<Sun size={18}/>:<Moon size={18}/>}</button></div>
    <header className="sp-heading"><div><span className="sp-kicker"><img src="/logo-transparent.png" alt="" width="22" height="22"/>Marina Social Club / The inspiration shelf</span><h1>Steal This</h1><p>Borrow the spark. Make something that’s yours.</p></div><button className="sp-text" onClick={onOpenCompetitors}>Manage competitors<ArrowRight size={16} aria-hidden="true"/></button></header>
    <div className="sp-feedback" role="status">{notice}</div>
    {(error||saved.error)&&<p id="sp-error" className="sp-error" role="alert">{error||saved.error}</p>}
    {route.postId?<>
      ...
      {!post?<section className="sp-empty"><h2 ref={titleRef} tabIndex={-1}>This example isn’t available.</h2><p>Return to Discover to choose an example from the preview library.</p></section>:<section className="sp-detail" aria-label="Adapt an example">
        <figure><IdeaArt post={post}/><figcaption>Illustrative artwork, not an imported social post.</figcaption><button className="sp-text" disabled={isSaved(post.id)||!!saved.error} onClick={()=>saveIdea(post)}>{isSaved(post.id)?<Check size={17} aria-hidden="true"/>:<Bookmark size={17} aria-hidden="true"/>}{isSaved(post.id)?'Idea saved':'Save idea for later'}</button></figure>
        <div className="sp-adapt"><span className="sp-eyebrow">{post.accountName} · {COMPETITOR_NETWORKS[post.platform]} · {monthName(post.month)}</span><h2 ref={titleRef} tabIndex={-1}>{post.title}</h2><p className="sp-reference-caption">{post.caption}</p>
          <details className="sp-source"><summary>About this example</summary><p>Fictional account and sample numbers. This is not a live ranking or proof that a format performs better.</p><div className="sp-example-stats"><span><Heart size={15} aria-hidden="true"/>{post.likes.toLocaleString()} sample likes</span><span><MessageCircle size={15} aria-hidden="true"/>{post.comments} sample comments</span></div></details>
          <div className="sp-takeaway"><span className="sp-eyebrow">The idea to borrow</span><p>{inspirationTakeaway(post)}</p></div>
          <form onSubmit={e=>{e.preventDefault();saveBrief(true);}}><div className="sp-brief-heading"><h3>Make it Marina.</h3><span>Your original direction</span></div><label htmlFor="sp-brief">Creative brief</label><textarea id="sp-brief" ref={briefRef} value={draft} rows={9} maxLength={6000} aria-invalid={error?true:undefined} aria-describedby={error?'sp-brief-help sp-error':'sp-brief-help'} onChange={e=>{setDrafts(current=>({...current,[post.id]:e.target.value}));setError('');setNotice('');}}/><p id="sp-brief-help" className="sp-help">Edit the idea for your client. Save to keep it in this browser. No content is generated or published here.</p><div className="sp-actions"><button type="submit" className="sp-button sp-primary">Continue in AI Studio<ArrowRight size={17} aria-hidden="true"/></button><button type="button" className="sp-text" onClick={()=>saveBrief(false)}><Bookmark size={16} aria-hidden="true"/>Save brief</button></div></form>
        </div>
      </section>}
    </>:<>
      <nav className="sp-tabs" aria-label="Inspiration library"><button aria-pressed={route.view==='discover'} onClick={()=>navigate({view:'discover'})}>Discover</button><button aria-pressed={route.view==='saved'} onClick={()=>navigate({view:'saved'})}><Bookmark size={16} aria-hidden="true"/>Saved ideas <span>{saved.ideas.length}</span></button></nav>
      <div className="sp-controls"><div className="sp-search"><label htmlFor="sp-search">Find an idea</label><div><Search size={17} aria-hidden="true"/><input id="sp-search" type="search" placeholder="Search ideas or accounts" maxLength={100} value={route.query} onChange={e=>navigate({query:e.target.value})}/></div></div>{route.view==='discover'?<><div className="sp-platforms" role="group" aria-label="Inspiration platform"><button aria-pressed={route.platform==='ig'} onClick={()=>navigate({platform:'ig',format:'all'})}><FaInstagram aria-hidden="true"/>Instagram</button><button aria-pressed={route.platform==='tt'} onClick={()=>navigate({platform:'tt',format:'all'})}><FaTiktok aria-hidden="true"/>TikTok</button></div><div className="sp-month"><label htmlFor="sp-month">Sample month</label><div><button className="sp-icon" aria-label="Previous inspiration month" disabled={route.month===COMPETITOR_MONTHS[0]} onClick={()=>navigate({month:COMPETITOR_MONTHS[0]})}><ChevronLeft size={18}/></button><select id="sp-month" value={route.month} onChange={e=>navigate({month:e.target.value})}>{COMPETITOR_MONTHS.map(month=><option key={month} value={month}>{monthName(month)}</option>)}</select><button className="sp-icon" aria-label="Next inspiration month" disabled={route.month===COMPETITOR_MONTHS[1]} onClick={()=>navigate({month:COMPETITOR_MONTHS[1]})}><ChevronRight size={18}/></button></div></div></>:<p className="sp-saved-hint">Kept across months and platforms.<br/>Shared with your Competitor Insights saved ideas.</p>}</div>
      <div className="sp-filterbar"><div role="group" aria-label="Content format">{INSPIRATION_FORMATS.filter(([key])=>route.view==='saved'||route.platform!=='tt'||['all','video'].includes(key)).map(([key,label])=><button key={key} aria-pressed={route.format===key} onClick={()=>navigate({format:key})}>{label}</button>)}</div><span role="status">{posts.length} {route.view==='saved'?'saved ideas':'examples'}</span></div>
      <section className="sp-gallery" ref={galleryRef} aria-label={route.view==='saved'?'Saved inspiration':'Content inspiration'}>{posts.map(item=><article key={item.id} className="sp-item"><button className="sp-open" data-idea-id={item.id} aria-label={`Explore ${item.title}`} onClick={()=>navigate({postId:item.id},true)}><IdeaArt post={item} decorative/><span className="sp-item-source">{item.accountName}<span>{COMPETITOR_NETWORKS[item.platform]}</span></span><h2>{item.title}</h2><span className="sp-item-action">Make it yours<ArrowRight size={17} aria-hidden="true"/></span></button><button className="sp-save sp-icon" aria-label={`${isSaved(item.id)?'Saved':'Save idea'}: ${item.title}`} disabled={isSaved(item.id)||!!saved.error} onClick={()=>saveIdea(item)}>{isSaved(item.id)?<Check size={18}/>:<Bookmark size={18}/>}</button></article>)}</section>
      {!posts.length&&<section className="sp-empty"><Bookmark size={27} aria-hidden="true"/><h2 ref={titleRef} tabIndex={-1}>{route.view==='saved'&&!saved.ideas.length?'Build your own inspiration shelf.':'No ideas match these filters.'}</h2><p>{route.view==='saved'&&!saved.ideas.length?'Save an example in Discover, then return to it when you are ready to create.':'Try another format or account, or clear your search to see the full selection.'}</p><button className="sp-button" onClick={()=>navigate({query:'',format:'all',...(!saved.ideas.length?{view:'discover'}:{})})}>{route.view==='saved'&&!saved.ideas.length?'Explore examples':'Clear filters'}</button></section>}
    </>}
    <footer className="sp-footer"><span>Ideas to adapt. Never content to copy.</span><span>Sample library · Saved in this browser only</span></footer>
  </main>;
}
