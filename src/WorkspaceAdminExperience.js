...
export function TeamExperience({liveMembers=null,pendingInvites=null,onInvite=null,onChangeRole=null,onRemoveMember=null}={}){
  ...
  // Which member the per-person menu is confirming a removal for.
  ...
  return <main className="wa-page">{!liveMembers&&<div className="wa-preview"><span>Team & permissions · Sample members</span><span><ShieldCheck size={14}/>Role-based access</span></div>}<Intro eyebrow="Agency team" title={<>The right access.<br/><em>Nothing more.</em></>} copy="Invite collaborators, set what they can see, and keep client work separated as the agency grows." actions={<button className="wa-primary" type="button" onClick={()=>setInviting(true)}><UserPlus size={16}/>Invite teammate</button>}/>
    ...
    <section className="wa-panel wa-team-panel"><header><div><span>People</span><h2>Workspace members.</h2></div><button type="button" onClick={()=>{const grid=document.getElementById('wa-permission-grid');if(grid){grid.scrollIntoView({behavior:'smooth',block:'start'});grid.focus({preventScroll:true});}}}>View permissions matrix</button></header><div className="wa-team-list">{members.map(member=><article key={member.email}><i>{member.initials}</i><span><strong>{member.name}</strong><small>{member.email}</small></span><label><span className="sr-only">Role for {member.name}</span><select aria-label={`Role for ${member.name}`} value={member.role} disabled={member.role==='Owner'} onChange={event=>{const next=event.target.value,was=member.role;setMembers(current=>current.map(item=>item.email===member.email?{...item,role:next}:item));if(!onChangeRole){notify(`${member.name} is now ${next}.`);return;}Promise.resolve(onChangeRole(member,next)).then(ok=>{if(ok){notify(`${member.name} is now ${next}.`);}else{setMembers(current=>current.map(item=>item.email===member.email?{...item,role:was}:item));notify(`Could not change ${member.name}'s role.`);}}).catch(()=>{setMembers(current=>current.map(item=>item.email===member.email?{...item,role:was}:item));notify(`Could not change ${member.name}'s role.`)})}}>{['Owner','Admin','Manager','Content creator','Analyst','Billing'].map(role=><option key={role}>{role}</option>)}</select><ChevronDown size={14}/></label><em><i/>Active</em>{member.role==='Owner'||!onRemoveMember?null:confirming===member.email?<span className="wa-team-confirm"><button type="button" onClick={()=>{const who=member;setConfirming(null);Promise.resolve(onRemoveMember(who)).then(ok=>{if(ok){setMembers(current=>current.filter(item=>item.email!==who.email));notify(`${who.name} was removed.`);}else notify(`Could not remove ${who.name}.`)}).catch(()=>notify(`Could not remove ${who.name}.`))}}>Remove</button><button type="button" onClick={()=>setConfirming(null)}>Cancel</button></span>:<button type="button" aria-label={`Remove ${member.name} from the workspace`} onClick={()=>setConfirming(member.email)}><MoreHorizontal size={17}/></button>}</article>)}</div></section>
